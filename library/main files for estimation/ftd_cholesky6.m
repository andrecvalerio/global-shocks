function [Ui,Vi,n0,np,ixmC0Pres] = ftd_cholesky6(lags,nvar,nexo,indxC0Pres)
%January 2016
% The model:
%   GDP CPI VIX PCOM CR EXR INTR
%
%   Identification structure: X means unrestricted and columns are
%   equations
%   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   %        GDP CPI WGDP PCOM  CR EXR INTR %
%   % GDP	  X	  X   0    0    0   0   X   %
%   % CPI	  0   X   0    0    0   0   X   %
%   % WGDP    X   X   X    X    X   X   X   %
%   % PCOM    X	  X   0    X    X   X   X   %
...
if (1)
 ...
 %--- Lag restrictions.
    indxeqn = 3; % Which equation: VIX
 	nrestrs = (nvar-2)*lags+1;  %Number of restrictions.
	vars_restr = [1 2 5 6 7];  %Variables that are restricted.
	blags = zeros(nrestrs,k);
	cnt = 0;
	for ki = 1:lags
	   for kj=vars_restr
  ...
  %--- Lag restrictions.
	...
	Ri(1:nrestrs,:,indxeqn) = blags;
     ...
     %--- Lag restrictions.
	indxeqn = 5;   %Which equation: CR
	nrestrs = (nvar-3)*lags+1;  %Number of restrictions.
	vars_restr = [1 2 6 7];  %Variables that are restricted.
	blags = zeros(nrestrs,k);
	cnt = 0;
	for ki = 1:lags
	   for kj=vars_restr
