...
which_shock=[3,4,5];  %variables that receive the shock - WGDP, PCOM, VIX
...
irf_adj(:,:,2) = irf(:,:,2);%.*(1.326154);

adj_low68(:,:,1) = irf_low68(:,:,1);%.*(2.186557);
adj_low68(:,:,2) = irf_low68(:,:,2);%.*(1.326154);

adj_up68(:,:,1) = irf_up68(:,:,1);%.*(2.186557);
adj_up68(:,:,2) = irf_up68(:,:,2);%.*(1.326154);

irf_adj(:,:,3)   = irf(:,:,3);%.*(-2.818251);
adj_low68(:,:,3) = irf_low68(:,:,3); %irf_up68(:,:,3).*(-2.818251);
adj_up68(:,:,3)  = irf_up68(:,:,3);%irf_low68(:,:,3).*(-2.818251);
...
vix_shock  = irf_adj(:,:,3);
pcom_shock = irf_adj(:,:,2);


wgdp_low68 = adj_low68(:,:,1);
vix_low68  = adj_low68(:,:,3);
pcom_low68 = adj_low68(:,:,2);


wgdp_up68 = adj_up68(:,:,1);
vix_up68  = adj_up68(:,:,3);
pcom_up68 = adj_up68(:,:,2);
