...
%
% To interpolate monthly chain-type GDP price index, use monthly data from xdm.mat on
...
%*    at the *orignal* (or untransformed) level. TZ, 5/7/97
vlist = [3 28 1 12];      %   M2, PCE price index, CPI-U, PPI (Finished Goods)
%vlist = [3 1 12];      %   M2, PCE price index, CPI-U, PPI (Finished Goods)
...
qmStart=1;
yrEnd=2007;    % <<>>1
qmEnd=9;    % <<>>1  Must end at the quarterly frequency.
...
trend = trend';
C = ones(nSample,1);
Xm=[C xdd(:,vlist)];  % monthly data on the right hand side
...
%xdd = xdd(:,2:end);  %Somehow Matlab reads the date column in the Excel file as NaN, thus we get rid of it.
[nt,ndv]=size(xdd);
if ndv~=42   %<<>>2
...
%* Do not log to ensure that the sum of montly variable = quarterly variabe at the *orignal*
%  level. TZ, 5/7/97
vlist = [3];    % Chain-type GDP price index.
...
%
if nEnd>0 | nStart<0
   disp('Warning: this particular sample considered is out of bounds of the data!!!')
   return
end
%
xdd=xdd(nStart+1:nt+nEnd,:);
baddata = find(~isfinite(xdd(:,vlist)));
%xddebt = xdd;
%xddebt(:,15) = ones(size(xdd,1),1);
%baddata = find(~isfinite(xddebt));
if ~isempty(baddata)
...
yqM = zeros(size(xdd,1),length(vlist));   % quarterly data
ymM = zeros(3*size(yqM,1),length(vlist));  % interpolated monthly data
for k=1:length(vlist)
   yq=xdd(:,vlist(k));  % quarterly data on the left hand side
   ...
   yqM(:,k) = yq;
   ymM(:,k) = Xm*bhat + ACt*(Vq\uqhat);
end

%*** generate monthly DGDP.
dgdpmon = ymM;
dgdpq = yqM;
%save outgdp dgdpmon dgdpq -append
