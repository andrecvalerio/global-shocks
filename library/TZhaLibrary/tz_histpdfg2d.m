function [xc,yc,z,zn] = tz_histpdfg2d(w,n,ditp,titstr,xlabstr,ylabstr,zlabstr,xlimvals,ylimvals,zlimvals)
...
% n: 1-by-2 vector -- # of bins (integers in general) for both x-axis and y-axis.  Suggested [50, 50];
% ditp: number -- degrees of bicubic interpolation.  Suggested value: 2.  If ditp = 0, NO bicubic interpolation.  
% titstr:  title string; if [], no title
% xlabstr:  xlabel string; if [], no xlab
% ylabstr:  ylabel string; if [], no ylab
% zlabstr:  ylabel string; if [], no ylab
% xlimvals:  [min max] limit on the x-axis;  if [], no xlim;
% ylimvals:  [min max] limit on the y-axis;  if [], no ylim;
% zlimvals:  [min max] limit on the y-axis;  if [], no zlim;
%---------------------------------
...
timeloop=toc;
disp(['Loop time for getting probability for each bin (minutes): ' num2str(timeloop/60)])
